...
起動するとトレイにキツネのアイコンが出ます。左クリックまたは右クリックでメニューが開き、ダブルクリックで設定画面が開きます。メニューには「全画面を撮影」「アクティブウィンドウを撮影」「設定...」「保存先を開く」「管理者として再起動」「終了」があります。「管理者として再起動」は通常権限で動いているときだけ表示され、管理者権限で動いているときは出ません。

<img src="docs/tray-menu.png" width="415" alt="トレイのキツネアイコンとメニュー（通常権限で起動したとき）">
...
<img src="docs/notification.png" width="420" alt="保存時の通知">
...
| First Fox Screenshot（ffshot、一番狐）の権限 | 登録先 | ログオン時の起動 |
|---|---|---|
| 通常権限 | `HKCU\...\Run` | 通常権限 |
