...
| Phase | 内容 | 完了条件 | 状態 |
|---|---|---|---|
| 1 | ソリューション作成、トレイ常駐、多重起動防止、設定の読み書き、終了メニュー | 起動するとトレイに出て終了できる | 完了 |
| 2 | `HotkeyManager`、設定画面でホットキー変更、登録失敗時の通知 | 設定したキーでイベントが飛ぶ | 完了 |
| 3 | GDI バックエンド（全画面・アクティブウィンドウ）、PNG 保存、通知 | 両ホットキーで PNG が保存される | 完了 |
| 4 | Desktop Duplication バックエンド、設定でバックエンド切替、GDI フォールバック | DirectX 全画面アプリが黒くならず撮れる | 完了 |
| 5 | 仕上げ：Windows 起動時登録（Run キー）、カーソル合成、複数モニター、テスト、README | | 完了 |

### 実装時に判明した点

- Desktop Duplication は `DuplicateOutput` 直後の最初の `AcquireNextFrame` が空フレーム（`LastPresentTime == 0`）で返るため、実フレームが来るまで解放・再取得する。
- デスクトップ面のアルファは不定なので、PNG に書く前に 255 に揃える。
- 動作確認は Windows 側で `SendKeys` によりホットキーを合成入力し、GDI / Direct3D の両方で全画面・アクティブウィンドウの PNG が保存されることを確認済み。
